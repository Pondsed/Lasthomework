class Tree<T>
{
    ...
    public Queue<T> ShowLeader(T value){
        TreeNode<T> showLeader = GetTreeNode(value);
        Queue<T> queueshowLeader = new Queue<T>();

        while(true){
            T SelectMember = showLeader.Leader().GetValue();
            queueshowLeader.Push(SelectMember);
            showLeader = showLeader.Leader();
            if(showLeader.Leader() == null ){
                break;
            }
        }
        return queueshowLeader;
    }
